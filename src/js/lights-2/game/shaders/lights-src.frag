...
float sceneDist(vec2 p)
{
	...
	float t2 = triangleDist(	rotateCW(translate(p, vec2(600, 250)), time), 40.0);
	...
	float b3 = boxDist(		translate(p, vec2(100, sin(time * 3.0 + 1.0) * 40.0 + 100.0)),
					   		vec2(40, 15), 	0.0);
	float c2 = circleDist(	translate(p, vec2(100, 100)),	30.0);
	float s = substract(b3, c2);

	float b4 = boxDist(		translate(p, vec2(200, sin(time * 3.0 + 2.0) * 40.0 + 100.0)),
					   		vec2(40, 15), 	0.0);
	float c3 = circleDist(	translate(p, vec2(200, 100)), 	30.0);
	float i = intersect(b4, c3);

	float b5 = boxDist(		translate(p, vec2(300, sin(time * 3.0 + 3.0) * 40.0 + 100.0)),
					   		vec2(40, 15), 	0.0);
...
void main() {
	...
	vec2 light1Pos = mouse.xy;
	vec4 light1Col = vec4(0.75, 1.0, 0.5, 1.0);
	setLuminance(light1Col, 0.4);

	vec2 light2Pos = vec2(resolution.x * (sin(time + 3.1415) + 1.2) / 2.4, 175.0);
	vec4 light2Col = vec4(1.0, 0.75, 0.5, 1.0);
	setLuminance(light2Col, 0.5);

	vec2 light3Pos = vec2(resolution.x * (sin(time) + 1.2) / 2.4, 340.0);
	vec4 light3Col = vec4(0.5, 0.75, 1.0, 1.0);
	setLuminance(light3Col, 0.6);


	// gradient
	vec4 col = vec4(0.5, 0.5, 0.5, 1.0) * (1.0 - length(c - p)/resolution.x);

	// ambient occlusion
	col *= AO(p, sceneSmooth(p, 10.0), 40.0, 0.4);
	//col *= AO(p, sceneDist(p), 40.0, 1.0);
	// light
